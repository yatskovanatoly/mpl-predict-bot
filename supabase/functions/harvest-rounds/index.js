...
export const getRoundGames = (html) => {
    ...
    parentDiv.querySelectorAll('table.championship').forEach((table) => {
        ...
        table.querySelectorAll('tbody tr.game').forEach((row) => {
            ...
            const game_id = Number(scoreEl?.getAttribute('href')?.split('/').at(-2) ?? 0);
            games.push({
                game_id,
                time,
...
Deno.serve(async () => {
    const url = 'https://mychamp.ru/championships/202/games?configuration[type]=games';
