...
import { Bot, Context, InlineKeyboard, InputFile, MemorySessionStorage, session, webhookCallback, } from 'grammy';
import { buildMainMenu, buildMenuButton, buildRoundMenu, } from './helpers/build-menus';
...
bot.callbackQuery('predict', async (ctx) => {
    ...
    const gamesWithPrediction = [];
    const gamesWithoutPrediction = [];
    ...
    if (!!gamesWithoutPrediction.length) {
        try {
            await ctx.editMessageText(`${!!usersPredictions.length
                ? ctx.t('predicted', { n: games.round })
                : ''}\n\n${usersPredictions
                .map(userPredictionIteratee)
                .join('\n')}\n\n${ctx.t('match_select')}`, {
                reply_markup: buildRoundMenu(ctx, games.games, usersPredictions.map((p) => p.game_id)),
            });
        }
        catch (err) {
            console.log(err);
...
bot.on('callback_query:data', async (ctx) => {
    ...
    if (data === 'leaderboard') {
        try {
            ...
            await editHelper(() => ctx.editMessageText(table, {
                reply_markup: buildMenuButton(ctx),
            }), ctx);
            return;
        }
    ...
    if (data === 'prev') {
        const usersPredictions = await getPredictionsByUser(ctx.from.id, games.round - 1);
        ctx.editMessageText(usersPredictions.map(userPredictionIteratee).join('\n'), { reply_markup: buildMenuButton(ctx) });
    }
});
bot.on('message:text', async (ctx) => {
    ...
    if (ctx.session.game) {
        ...
        try {
            await saveUserPrediction(id, ctx, home.team, away.team, score, round).then(async () => {
                await ctx.reply(ctx.t('prediction_made', {
                    n: '\n\n',
                    home: home.team,
                    away: away.team,
                    homeGoals: homeGoals,
                ...
                const predicted = await getPredictionsByUser(ctx.from.id, round).then((data) => data.map((p) => p.game_id));
            ...
            await ctx.reply(ctx.t('prediction_fail', {
